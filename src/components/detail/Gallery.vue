  ...
  <section id="gallery">
    ...
    <div class="grid grid-cols-4 gap-4 mt-4">
      <template v-for="gallery in galleries" :key="gallery.id">
        <div @click="changeImage(gallery.image)" class="overflow-hidden cursor-pointer rounded-2xl" :class="{'ring-2 ring-yellow-500' : defaultImage == gallery.image}">
        <img :src="'/src/assets/img/' + gallery.image" class="w-full" alt="" />
      </div>
      </template>
      <!-- <div class="overflow-hidden cursor-pointer ring-2 ring-yellow-500 rounded-2xl">
